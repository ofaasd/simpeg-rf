...
$(function () {
  ...
  var dt_table = $('.datatables-' + page),
    ...
    offCanvasForm = $('#offcanvasAdd' + page);
  if (select2.length) {
    var $this = select2;
  ...
  if (dt_table.length) {
    var dt = dt_table.DataTable({
      ...
      columnDefs: [{
        ...
        orderable: false,
        render: function render(data, type, full, meta) {
          return '<div class="d-inline-block text-nowrap">' + '<button class="btn btn-sm btn-icon edit-record" data-id="'.concat(full['id'], '" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAdd').concat(title, '"><i class="mdi mdi-pencil-outline mdi-20px"></i></button>') + '<button class="btn btn-sm btn-icon delete-record" data-id="'.concat(full['id'], '"><i class="mdi mdi-delete-outline mdi-20px"></i></button>');
        }
      }],
      order: [[2, 'desc']],
      ...
      buttons: [{
        ...
        text: '<i class="mdi mdi-plus me-0 me-sm-2"></i><span class="d-none d-sm-inline-block">Add New ' + title + '</span>',
        className: 'add-new btn btn-primary',
        attr: {
          'data-bs-toggle': 'offcanvas',
          'data-bs-target': '#offcanvasAdd' + page
        }
      }]
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    $.get(''.concat(baseUrl).concat(page, '/').concat(id, '/edit'), function (data) {
      Object.keys(data).forEach(function (key) {
        //console.log(key);
        if (key == 'id') $('#' + page + '_' + key).val(data[key]);else $('#add-' + page + '-' + key).val(data[key]).trigger('change');
      });
    });
  });
  ...
  $('.add-new').on('click', function () {
    ...
    $('#offcanvasAdd' + page + 'Label').html('Add ' + title);
  });

  // validating form and updating data
  var addNewForm = document.getElementById('addNew' + page + 'Form');
  ...
  var fv = FormValidation.formValidation(addNewForm, {
    ...
    var formElement = $('#addNew' + page + 'Form');
